      ...
      <div className="max-w-7xl mx-auto p-6">
        ...
        <div className="mb-8">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              <h1 className="text-3xl font-black text-gray-900 tracking-tight">Beban DTPR (1.A.4)</h1>
              <p className="text-gray-500 mt-1 font-medium">Monitoring beban kerja dosen tetap program studi (EWMP)</p>
            </div>
        ...
        <div className="flex flex-col lg:flex-row gap-6 mb-8">
          <div className="flex-1 grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-2xl font-black text-gray-900">{summary ? parseFloat(summary.avg_total).toFixed(1) : '0'}</p>
              </div>
            </div>
            <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 flex items-center gap-4">
          ...
          <div className="flex gap-3 items-end">
            <div className="flex-1 lg:w-48">
              ...
              <select value={filterIdProdi} onChange={(e) => setFilterIdProdi(e.target.value)} className="w-full px-4 py-2.5 bg-white border border-gray-200 rounded-xl focus:ring-4 focus:ring-blue-100 outline-none font-bold text-sm transition cursor-pointer">
            ...
            <button onClick={() => setIsTrashMode(!isTrashMode)} className={`px-4 py-2.5 rounded-xl font-bold text-sm transition border shadow-sm ${isTrashMode ? 'bg-orange-50 border-orange-200 text-orange-600' : 'bg-white border-gray-200 text-gray-600 hover:bg-gray-50'}`}>
        ...
        {showForm && (
          <div className="bg-white rounded-3xl shadow-xl shadow-gray-200/50 border border-gray-100 p-8 mb-8 animate-in slide-in-from-top-4 duration-500">
            <h2 className="text-xl font-black text-gray-900 mb-6">{editingId ? 'Edit Beban Kerja' : 'Input Beban Kerja Baru'}</h2>
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <div className="md:col-span-2 lg:col-span-2">
                  <label className="block text-sm font-bold text-gray-700 mb-2">Pilih DTPR (Dosen)</label>
                  <select value={formData.id_dosen} onChange={(e) => setFormData({...formData, id_dosen: e.target.value})} className="w-full px-4 py-3 bg-gray-50 border-transparent border-2 focus:border-blue-500 focus:bg-white rounded-2xl outline-none transition font-medium" required>
                    <option value="">Pilih Dosen</option>
                    {dosenList.map(d => <option key={d.id_dosen} value={d.id_dosen}>{d.nama_lengkap}</option>)}
                  </select>
                </div>
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-2">SKS PS Sendiri</label>
                  <input type="number" step="0.01" value={formData.sks_ps_sendiri} onChange={(e) => setFormData({...formData, sks_ps_sendiri: e.target.value})} className="w-full px-4 py-3 bg-gray-50 border-transparent border-2 focus:border-blue-500 focus:bg-white rounded-2xl outline-none transition font-medium" placeholder="0.00" />
                </div>
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-2">SKS PS Lain</label>
                  <input type="number" step="0.01" value={formData.sks_ps_lain} onChange={(e) => setFormData({...formData, sks_ps_lain: e.target.value})} className="w-full px-4 py-3 bg-gray-50 border-transparent border-2 focus:border-blue-500 focus:bg-white rounded-2xl outline-none transition font-medium" placeholder="0.00" />
                </div>
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-2">SKS PT Lain</label>
                  <input type="number" step="0.01" value={formData.sks_pt_lain} onChange={(e) => setFormData({...formData, sks_pt_lain: e.target.value})} className="w-full px-4 py-3 bg-gray-50 border-transparent border-2 focus:border-blue-500 focus:bg-white rounded-2xl outline-none transition font-medium" placeholder="0.00" />
                </div>
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-2">SKS Penelitian</label>
                  <input type="number" step="0.01" value={formData.sks_penelitian} onChange={(e) => setFormData({...formData, sks_penelitian: e.target.value})} className="w-full px-4 py-3 bg-gray-50 border-transparent border-2 focus:border-blue-500 focus:bg-white rounded-2xl outline-none transition font-medium" placeholder="0.00" />
                </div>
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-2">SKS PkM</label>
                  <input type="number" step="0.01" value={formData.sks_pkm} onChange={(e) => setFormData({...formData, sks_pkm: e.target.value})} className="w-full px-4 py-3 bg-gray-50 border-transparent border-2 focus:border-blue-500 focus:bg-white rounded-2xl outline-none transition font-medium" placeholder="0.00" />
                </div>
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-2">SKS Manajemen PT Lain</label>
                  <input type="number" step="0.01" value={formData.sks_manajemen_pt_lain} onChange={(e) => setFormData({...formData, sks_manajemen_pt_lain: e.target.value})} className="w-full px-4 py-3 bg-gray-50 border-transparent border-2 focus:border-blue-500 focus:bg-white rounded-2xl outline-none transition font-medium" placeholder="0.00" />
                </div>
              </div>
              ...
              <table className="w-full text-left border-collapse">
                <thead className="bg-gray-50/50 border-b border-gray-100">
                  <tr>
                    ...
                    <th rowSpan="2" className="px-8 py-4 text-[11px] font-black text-gray-900 uppercase tracking-[0.2em] border-r border-gray-100 align-middle">Nama DTPR</th>
                    <th colSpan="3" className="px-6 py-4 text-[10px] font-black text-gray-400 uppercase tracking-widest border-r border-gray-100 text-center bg-gray-50/30">SKS Pengajaran</th>
                    <th rowSpan="2" className="px-4 py-4 text-[10px] font-black text-gray-400 uppercase tracking-widest border-r border-gray-100 text-center align-middle">Riset</th>
                    <th rowSpan="2" className="px-4 py-4 text-[10px] font-black text-gray-400 uppercase tracking-widest border-r border-gray-100 text-center align-middle">PkM</th>
                    <th colSpan="2" className="px-6 py-4 text-[10px] font-black text-gray-400 uppercase tracking-widest border-r border-gray-100 text-center bg-gray-50/30">SKS Manajemen</th>
                    <th rowSpan="2" className="px-6 py-4 text-[11px] font-black text-blue-600 uppercase tracking-[0.2em] border-r border-gray-100 text-center align-middle bg-blue-50/50">Total</th>
                  ...
                  <tr className="border-b border-gray-100 bg-gray-50/20">
                    <th className="px-3 py-3 text-[9px] font-black text-gray-400 uppercase tracking-widest border-r border-gray-100 text-center">PS Sendiri</th>
                    <th className="px-3 py-3 text-[9px] font-black text-gray-400 uppercase tracking-widest border-r border-gray-100 text-center">PS Lain</th>
                    <th className="px-3 py-3 text-[9px] font-black text-gray-400 uppercase tracking-widest border-r border-gray-100 text-center">PT Lain</th>
                    <th className="px-3 py-3 text-[9px] font-black text-gray-400 uppercase tracking-widest border-r border-gray-100 text-center">PT Sendiri</th>
                    <th className="px-3 py-3 text-[9px] font-black text-gray-400 uppercase tracking-widest border-r border-gray-100 text-center">PT Lain</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-50">
                  {(isTrashMode ? trashData : activeData).map((item, index) => (
                    <tr key={item.id_beban_kerja} className="hover:bg-blue-50/30 transition-colors group">
                      <td className="px-6 py-5 border-r border-gray-50 text-center text-[11px] font-black text-gray-300">{index + 1}</td>
                      <td className="px-8 py-5 border-r border-gray-50">
                        <div className="text-sm font-black text-gray-900 group-hover:text-blue-600 transition-colors">{item.nama_dtpr}</div>
                        <div className="text-[10px] text-gray-400 font-bold uppercase tracking-wider">{item.nama_prodi}</div>
                      </td>
                      <td className="px-4 py-5 border-r border-gray-50 text-center text-sm font-bold text-gray-600">{item.sks_ps_sendiri || '-'}</td>
                      <td className="px-4 py-5 border-r border-gray-50 text-center text-sm font-bold text-gray-600">{item.sks_ps_lain || '-'}</td>
                      <td className="px-4 py-5 border-r border-gray-50 text-center text-sm font-bold text-gray-600">{item.sks_pt_lain || '-'}</td>
                      <td className="px-4 py-5 border-r border-gray-50 text-center text-sm font-bold text-emerald-600">{item.sks_penelitian || '-'}</td>
                      <td className="px-4 py-5 border-r border-gray-50 text-center text-sm font-bold text-purple-600">{item.sks_pkm || '-'}</td>
                      <td className="px-4 py-5 border-r border-gray-50 text-center text-sm font-bold text-indigo-600">{item.sks_manajemen_pt_sendiri || '-'}</td>
                      <td className="px-4 py-5 border-r border-gray-50 text-center text-sm font-bold text-gray-600">{item.sks_manajemen_pt_lain || '-'}</td>
                      <td className="px-6 py-5 border-r border-gray-50 text-center bg-blue-50/20">
                      ...
                      <td className="px-8 py-5">
                        <div className="flex justify-center">
                          <div className="inline-flex items-center bg-white border border-gray-200 p-1.5 rounded-xl shadow-sm transition-all group-hover:border-blue-200 group-hover:shadow-md">
                            {isTrashMode ? (
                              <>
                                <button onClick={() => handleRestore(item.id_beban_kerja)} className="p-1.5 text-emerald-600 hover:bg-emerald-50 rounded-lg transition" title="Restore"><RotateCcw size={16} /></button>
                                <div className="w-px h-4 bg-gray-200 mx-2"></div>
                                <button onClick={() => handleHardDelete(item.id_beban_kerja)} className="p-1.5 text-red-600 hover:bg-red-50 rounded-lg transition" title="Hapus Permanen"><Trash size={16} /></button>
                              </>
                            ) : (
                              <>
                                ...
                                <div className="w-px h-4 bg-gray-200 mx-2"></div>
                                <button onClick={() => handleDelete(item.id_beban_kerja)} className="p-1.5 text-red-600 hover:bg-red-50 rounded-lg transition" title="Hapus"><Trash2 size={16} /></button>
                              </>
                            )}
                          </div>
                        </div>
                ...
                {summary && !isTrashMode && (
                  <tfoot className="bg-gray-50/50 font-black border-t-2 border-gray-100">
                    <tr className="text-gray-900">
                      ...
                      <td className="px-6 py-5 text-center bg-blue-100/50 text-blue-700 font-black text-sm">{parseFloat(summary.sum_total || 0).toFixed(1)}</td>
                      <td></td>
                    </tr>
                    <tr className="text-gray-500 bg-white/50 italic border-t border-gray-100">
                      ...
                      <td className="px-6 py-4 text-center bg-blue-50/50 text-blue-600 font-black text-xs">{parseFloat(summary.avg_total || 0).toFixed(1)}</td>
                      <td></td>
                    </tr>
                  </tfoot>
                )}
