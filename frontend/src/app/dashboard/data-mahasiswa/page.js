  ...
  return (
    <div className="min-h-screen bg-gray-50/50">
      <div className="max-w-7xl mx-auto p-6">
              ...
              <div className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  ...
                  <div>
                    ...
                    <select value={formData.id_tahun} onChange={(e) => setFormData({ ...formData, id_tahun: e.target.value })} disabled={true} className="w-full px-4 py-3 bg-gray-100 border-transparent border-2 rounded-2xl outline-none font-medium opacity-70 cursor-not-allowed" required>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left">
                <thead className="bg-gray-50/50 border-b border-gray-100">
                  ...
                  <tr className="text-[11px] font-black text-gray-500 text-center">
                    <th rowSpan="2" className="px-6 py-3 border-r border-gray-100 align-middle">Pendaftar</th>
                    <th rowSpan="2" className="px-6 py-3 border-r border-gray-100 align-middle">Pendaftar<br/>Afirmasi</th>
                    <th rowSpan="2" className="px-6 py-3 border-r border-gray-100 align-middle">Pendaftar<br/>Kebutuhan<br/>Khusus</th>
                      ...
                      if (item) {
                        rows.push(
                          <tr key={item.id_2a1} className="hover:bg-blue-50/30 transition-colors group">
                            <td className="px-8 py-4 border-r border-gray-100 text-center font-black text-gray-900">{tsLabel} <span className="text-[9px] text-gray-400 block font-normal">({item.tahun})</span></td>
                        ...
                        rows.push(
                          <tr key={`empty-${targetYear}`} className="bg-gray-50/30 hover:bg-gray-100 transition-colors group">
                            <td className="px-8 py-4 border-r border-gray-100 text-center font-black text-gray-500">{tsLabel} <span className="text-[9px] text-gray-400 block font-normal">({targetYear})</span></td>
                            <td className="px-8 py-4 border-r border-gray-100 text-center font-bold text-gray-400">0</td>
                            <td className="px-8 py-4 border-r border-gray-100 text-center text-gray-400">0</td>
                            <td className="px-8 py-4 border-r border-gray-100 text-center text-gray-400">0</td>
                            <td className="px-8 py-4 border-r border-gray-100 text-center text-gray-400">0</td>
                            <td className="px-8 py-4 border-r border-gray-100 text-center text-gray-400">0</td>
                            <td className="px-8 py-4 border-r border-gray-100 text-center text-gray-400">0</td>
                ...
                <tfoot className="bg-amber-50">
                  <tr className="text-center font-black text-gray-800 border-t-2 border-amber-200">
                    <td className="px-8 py-5 border-r border-amber-100">Jumlah</td>
                    <td className="px-8 py-5 border-r border-amber-100">
                      {data.filter(item => {
                        const selectedTahunObj = tahunList.find(t => t.id_tahun.toString() === filterTahun.toString());
                        const anchorYear = selectedTahunObj ? parseInt(selectedTahunObj.tahun) || new Date().getFullYear() : new Date().getFullYear();
                    ...
                    <td className="px-8 py-5 border-r border-amber-100 bg-amber-100/30">
                      {data.filter(item => {
                        const selectedTahunObj = tahunList.find(t => t.id_tahun.toString() === filterTahun.toString());
                        const anchorYear = selectedTahunObj ? parseInt(selectedTahunObj.tahun) || new Date().getFullYear() : new Date().getFullYear();
                        return parseInt(item.tahun) <= anchorYear && parseInt(item.tahun) >= anchorYear - 2;
                    ...
                    <td className="px-8 py-5 bg-white"></td>
                  </tr>
                </tfoot>
              </table>
            </div>
        ...
        {/* Trash Section */}
        {viewMode === 'trash' && (
          <div className="bg-red-50 rounded-3xl p-8 border border-red-100 shadow-sm animate-in slide-in-from-bottom-4 duration-500">
            <div className="flex items-center gap-4 mb-6">
              <div className="w-12 h-12 bg-red-100 text-red-600 rounded-xl flex items-center justify-center">
                <Trash2 size={24} />
              </div>
              <div>
                <h3 className="text-xl font-black text-red-700 tracking-tight">Tempat Sampah</h3>
                <p className="text-sm text-red-500 font-medium">Data yang dihapus sementara dapat dikembalikan atau dihapus permanen.</p>
              </div>
            </div>
            
            <div className="bg-white rounded-2xl shadow-xl shadow-red-100/50 overflow-hidden border border-red-100">
              <div className="overflow-x-auto">
                <table className="w-full text-sm text-left">
                  ...
                  <tbody className="divide-y divide-gray-50">
                    {trashData.length === 0 ? (
                      ...
                      trashData.map((item) => (
                        <tr key={item.id_2a1} className="hover:bg-red-50/30 transition-colors">
                          <td className="px-6 py-4 font-black text-gray-900">{item.tahun}</td>
                          <td className="px-6 py-4 text-gray-500 font-medium">{new Date().toLocaleDateString('id-ID')}</td>
                          <td className="px-6 py-4 flex items-center justify-center gap-3">
                            <button
                              onClick={() => handleRestore(item.id_2a1)}
                              className="px-4 py-2 bg-blue-600 text-white rounded-xl font-bold text-xs hover:bg-blue-700 transition uppercase shadow-lg shadow-blue-200"
                            >
                              Restore
                            </button>
                            <button
                              onClick={() => handleHardDelete(item.id_2a1)}
                              className="px-4 py-2 bg-red-50 text-red-600 border border-red-100 rounded-xl font-bold text-xs hover:bg-red-600 hover:text-white transition uppercase"
                            >
                              Hard Delete
                            </button>
                          </td>
