      ...
      <div className="max-w-7xl mx-auto p-6">
        ...
        <div className="mb-8">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              <h1 className="text-3xl font-black text-white tracking-tight">Isi Pembelajaran (2.B.1)</h1>
              <p className="text-gray-400 mt-1 font-medium">Pengelolaan mata kuliah dan profil lulusan</p>
            </div>
            <div className="flex flex-wrap gap-3">
              <button onClick={() => window.open(`http://localhost:5000/api/prodi/2b1-isi-pembelajaran/export?id_prodi=${filterProdi}&token=${localStorage.getItem('token')}`, '_blank')} className="flex items-center gap-2 bg-emerald-600 text-white px-5 py-2.5 rounded-xl hover:bg-emerald-700 transition shadow-lg shadow-emerald-900/20 font-bold text-sm">
                <Download size={18} />
          ...
          <div className="bg-gray-900 rounded-3xl shadow-xl shadow-gray-950/50 border border-gray-700 p-8 mb-8 animate-in slide-in-from-top-4 duration-500">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="space-y-6">
                <div>
                  <label className="block text-sm font-bold text-gray-300 mb-2">Mata Kuliah</label>
                    ...
                    <table className="w-full text-sm">
                      <thead className="bg-gray-800 sticky top-0">
                        <tr>
                          ...
                          <th className="px-4 py-2 text-left font-medium text-gray-300">Mata Kuliah</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-gray-800">
                        {editingMk ? (
                          ...
                          >
                            <td className="px-4 py-2 font-mono text-xs font-bold text-gray-200">{editingMk.kode_mk}</td>
                            <td className="px-4 py-2 text-gray-400">{editingMk.nama_mk}</td>
                          </tr>
                        ) : (
                          mataKuliahList.map(mk => (
                          <tr 
                            key={mk.id_mk} 
                            className={`hover:bg-blue-900/20 cursor-pointer ${editingMk?.id_mk === mk.id_mk ? 'bg-blue-900/30' : ''}`}
                            onClick={() => {
                              setEditingMk(mk);
                              setSelectedPls([]);
                            }}
                          >
                            <td className="px-4 py-2 font-mono text-xs font-bold text-gray-200">{mk.kode_mk}</td>
                            <td className="px-4 py-2 text-gray-400">{mk.nama_mk}</td>
                          </tr>
                        ))
                        )}
                      </tbody>
                ...
                <tbody className="divide-y divide-gray-800">
                  {mataKuliahList.map((mk) => {
                    ...
                    if (mappedPls.length === 0) {
                      ...
                      return (
                        <tr key={mk.id_mk} className="hover:bg-blue-900/30 transition-colors">
                          <td className="px-4 py-3 border-r border-gray-700 bg-gray-950/50 align-top sticky left-0 z-10 w-[260px] min-w-[260px] max-w-[260px] shadow-[inset_-1px_0_0_0_#1f2937]">
                            <div className="font-black text-white text-sm">{mk.nama_mk || '-'}</div>
                            <div className="text-[10px] text-gray-400 mt-1 font-medium">{mk.kode_mk || '-'}</div>
                          </td>
