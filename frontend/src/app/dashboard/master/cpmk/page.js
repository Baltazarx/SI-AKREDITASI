      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              <h1 className="text-3xl font-black text-white tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-white to-gray-500">Master CPMK</h1>
              <p className="text-gray-400 mt-1 font-medium italic">Kelola Capaian Pembelajaran Mata Kuliah (CPMK) sesuai standar kurikulum Prodi</p>
            </div>
        ...
        {showForm && (
          <div className="bg-gray-900 rounded-3xl shadow-xl shadow-gray-950/50 border border-gray-700 p-8 mb-8 animate-in slide-in-from-top-4 duration-500">
            <h2 className="text-xl font-black text-white mb-6 flex items-center gap-2">
              ...
              {editingId ? 'Edit Data CPMK' : 'Input Data CPMK Baru'}
            </h2>
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="grid grid-cols-1 gap-6">
                ...
                <div>
                  <label className="block text-sm font-bold text-gray-300 mb-2">Deskripsi CPMK</label>
                  <div className="relative">
                    <textarea 
              ...
              <div className="flex justify-end gap-3 pt-4 border-t border-gray-700">
                ...
                <button type="submit" className="px-10 py-3 bg-blue-600 text-white rounded-2xl hover:bg-blue-700 transition font-black shadow-lg shadow-blue-900/20">{editingId ? 'Simpan Perubahan' : 'Tambah CPMK'}</button>
              </div>
            </form>
          </div>
        )}
        ...
        <div className="bg-gray-900 rounded-[2.5rem] shadow-xl shadow-gray-950/30 border border-gray-700 overflow-hidden">
          {loading ? (
            ...
            <div className="p-20 text-center flex flex-col items-center">
              ...
              <p className="text-gray-400 font-bold text-xl tracking-tight">Belum ada data CPMK untuk prodi ini</p>
            </div>
          ) : (
              ...
              <table className="w-full text-left border-collapse">
                <thead className="bg-gray-800 border-b border-gray-700">
                  <tr>
                    ...
                    <th className="px-4 py-5 text-[11px] font-black text-gray-300 uppercase tracking-[0.2em] border-r border-gray-700 w-44 text-center">Kode CPMK</th>
                    <th className="px-8 py-5 text-[11px] font-black text-gray-300 uppercase tracking-[0.2em] border-r border-gray-700">Deskripsi Capaian Pembelajaran Mata Kuliah (CPMK)</th>
                    <th className="px-8 py-5 text-[11px] font-black text-gray-300 uppercase tracking-[0.2em] text-center w-32">Aksi</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-800">
                  {data.map((item, index) => (
                    <tr key={item.id_cpmk} className="hover:bg-blue-900/10 transition-colors group">
                      ...
                      <td className="px-8 py-6 border-r border-gray-700 font-bold text-white group-hover:text-blue-400 transition-colors leading-relaxed">{item.deskripsi_cpmk}</td>
                      <td className="px-8 py-6">
                        <div className="flex justify-center gap-1">
                          ...
                          <button onClick={() => handleDelete(item.id_cpmk)} className="p-2 text-red-600 hover:bg-red-950/40 rounded-xl transition" title="Hapus"><Trash2 size={16} /></button>
                        </div>
                      </td>
                    </tr>
                  ))}
