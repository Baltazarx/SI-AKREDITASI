      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              <h1 className="text-3xl font-black text-white tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-white to-gray-500">Master Mata Kuliah</h1>
              <p className="text-gray-400 mt-1 font-medium italic">Kelola daftar mata kuliah kurikulum Program Studi</p>
            </div>
        ...
        {showForm && (
          <div className="bg-gray-900 rounded-3xl shadow-xl shadow-gray-950/50 border border-gray-700 p-8 mb-8 animate-in slide-in-from-top-4 duration-500">
            <h2 className="text-xl font-black text-white mb-6 flex items-center gap-2">
              ...
              {editingId ? 'Edit Data Mata Kuliah' : 'Input Data Mata Kuliah Baru'}
            </h2>
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  <label className="block text-sm font-bold text-gray-300 mb-2">Kode Mata Kuliah</label>
                  <div className="relative">
                    <input type="text" value={formData.kode_mk} onChange={(e) => setFormData({...formData, kode_mk: e.target.value})} className="w-full px-4 py-3 bg-gray-950/50 border-transparent border-2 focus:border-blue-500 focus:bg-gray-900 rounded-2xl outline-none transition font-medium text-white pl-11" placeholder="Contoh: MK001" required />
                    <Hash size={18} className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-500" />
                  </div>
                </div>
                <div>
                  <label className="block text-sm font-bold text-gray-300 mb-2">Nama Mata Kuliah</label>
                  <div className="relative">
                    <input type="text" value={formData.nama_mk} onChange={(e) => setFormData({...formData, nama_mk: e.target.value})} className="w-full px-4 py-3 bg-gray-950/50 border-transparent border-2 focus:border-blue-500 focus:bg-gray-900 rounded-2xl outline-none transition font-medium text-white pl-11" placeholder="Nama Mata Kuliah" required />
                    <Type size={18} className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-500" />
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-bold text-gray-300 mb-2">SKS</label>
                    <input type="number" value={formData.sks} onChange={(e) => setFormData({...formData, sks: e.target.value})} className="w-full px-4 py-3 bg-gray-950/50 border-transparent border-2 focus:border-blue-500 focus:bg-gray-900 rounded-2xl outline-none transition font-medium text-white" placeholder="2" required />
                  </div>
                  <div>
                    <label className="block text-sm font-bold text-gray-300 mb-2">Semester</label>
                    <input type="number" value={formData.semester} onChange={(e) => setFormData({...formData, semester: e.target.value})} className="w-full px-4 py-3 bg-gray-950/50 border-transparent border-2 focus:border-blue-500 focus:bg-gray-900 rounded-2xl outline-none transition font-medium text-white" placeholder="1" required />
                  </div>
                </div>
              </div>
              <div className="flex justify-end gap-3 pt-4 border-t border-gray-700">
                ...
                <button type="submit" className="px-10 py-3 bg-blue-600 text-white rounded-2xl hover:bg-blue-700 transition font-black shadow-lg shadow-blue-900/20">{editingId ? 'Simpan Perubahan' : 'Tambah Mata Kuliah'}</button>
              </div>
            </form>
          </div>
        )}
        ...
        <div className="bg-gray-900 rounded-[2.5rem] shadow-xl shadow-gray-950/30 border border-gray-700 overflow-hidden">
          {loading ? (
            ...
            <div className="p-20 text-center flex flex-col items-center">
              ...
              <p className="text-gray-400 font-bold text-xl tracking-tight">Belum ada data mata kuliah untuk prodi ini</p>
            </div>
          ) : (
              ...
              <table className="w-full text-left border-collapse">
                <thead className="bg-gray-800 border-b border-gray-700">
                  <tr>
                    ...
                    <th className="px-8 py-5 text-[11px] font-black text-gray-300 uppercase tracking-[0.2em] border-r border-gray-700 w-32 text-center">Kode MK</th>
                    <th className="px-8 py-5 text-[11px] font-black text-gray-300 uppercase tracking-[0.2em] border-r border-gray-700">Nama Mata Kuliah</th>
                    <th className="px-8 py-5 text-[11px] font-black text-gray-300 uppercase tracking-[0.2em] border-r border-gray-700 text-center w-24">SKS</th>
                    <th className="px-8 py-5 text-[11px] font-black text-gray-300 uppercase tracking-[0.2em] border-r border-gray-700 text-center w-24">Semester</th>
                    <th className="px-8 py-5 text-[11px] font-black text-gray-300 uppercase tracking-[0.2em] text-center w-32">Aksi</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-800">
                  {data.map((item, index) => (
                    <tr key={item.id_mk} className="hover:bg-blue-900/10 transition-colors group">
                      <td className="px-8 py-6 border-r border-gray-700 text-center font-bold text-gray-500">{index + 1}</td>
                      <td className="px-8 py-6 border-r border-gray-700 text-center font-black text-blue-500 text-xs tracking-wider">{item.kode_mk}</td>
                      <td className="px-8 py-6 border-r border-gray-700 font-bold text-white group-hover:text-blue-400 transition-colors">{item.nama_mk}</td>
                      <td className="px-8 py-6 border-r border-gray-700 text-center whitespace-nowrap">
                        <span className="px-3 py-1 bg-gray-800 rounded-lg text-xs font-black text-gray-300 border border-gray-700">{item.sks} SKS</span>
                      </td>
                      <td className="px-8 py-6 border-r border-gray-700 text-center">
                        <div className="flex items-center justify-center gap-2">
                           <Layers size={14} className="text-gray-500" />
                           <span className="text-sm font-bold text-gray-300">Smt {item.semester}</span>
                        </div>
                      </td>
                      <td className="px-8 py-6">
                        <div className="flex justify-center gap-1">
                          ...
                          <button onClick={() => handleDelete(item.id_mk)} className="p-2 text-red-600 hover:bg-red-950/40 rounded-xl transition" title="Hapus"><Trash2 size={16} /></button>
                        </div>
                      </td>
                    </tr>
                  ))}
