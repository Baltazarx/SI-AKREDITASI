      ...
      <div className="max-w-7xl mx-auto p-6">
        ...
        <div className="mb-8">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              <h1 className="text-3xl font-black text-gray-900 tracking-tight">Penggunaan Dana (1.A.3)</h1>
              <p className="text-gray-500 mt-1 font-medium">Monitoring alokasi dan penggunaan dana program studi</p>
            </div>
          ...
          <div className="flex-1 grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-2xl font-black text-gray-900">{formatRupiah(pivotedDataInfo.data.length ? pivotedDataInfo.data.reduce((acc, curr) => acc + (curr.ts || 0), 0) / pivotedDataInfo.data.length : 0)}</p>
              </div>
            </div>
            <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 flex items-center gap-4">
        ...
        {showForm && (
          <div className="bg-white rounded-3xl shadow-xl shadow-gray-200/50 border border-gray-100 p-8 mb-8 animate-in slide-in-from-top-4 duration-500">
            <h2 className="text-xl font-black text-gray-900 mb-6">{editingId ? 'Edit Penggunaan Dana' : 'Input Penggunaan Baru'}</h2>
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="md:col-span-2">
                  <label className="block text-sm font-bold text-gray-700 mb-2">Nama Penggunaan Dana</label>
                  <input type="text" value={formData.nama_penggunaan} onChange={(e) => setFormData({...formData, nama_penggunaan: e.target.value})} className="w-full px-4 py-3 bg-gray-50 border-transparent border-2 focus:border-blue-500 focus:bg-white rounded-2xl outline-none transition font-medium" placeholder="Contoh: Pembelian Alat Laboratorium" required />
                </div>
                <div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left">
                <thead className="bg-gray-50/50 border-b border-gray-100">
                  {showTrash ? (
                    <tr>
                      <th className="px-8 py-5 text-[11px] font-black text-gray-400 uppercase tracking-[0.2em] border-r border-gray-100">Penggunaan</th>
                    ...
                    <tr>
                      <th className="px-8 py-5 text-[11px] font-black text-gray-400 uppercase tracking-[0.2em] border-r border-gray-100">Penggunaan Dana</th>
                ...
                <tbody className="divide-y divide-gray-50">
                  {showTrash ? (
                    trashData.map((item) => (
                      <tr key={item.id_penggunaan} className="hover:bg-orange-50/30 transition-colors group">
                        <td className="px-8 py-6 border-r border-gray-50 last:border-0">
                          <div className="text-sm font-black text-gray-900">{item.nama_penggunaan}</div>
                        </td>
                        ...
                        <td className="px-8 py-6">
                          <div className="inline-flex items-center bg-white border border-gray-200 p-1.5 rounded-xl shadow-sm">
                            <button onClick={() => handleRestore(item.id_penggunaan)} className="p-1.5 text-emerald-600 hover:bg-emerald-50 rounded-lg transition" title="Restore"><RotateCcw size={16} /></button>
                            <div className="w-px h-4 bg-gray-200 mx-2"></div>
                            <button onClick={() => handleHardDelete(item.id_penggunaan)} className="p-1.5 text-red-600 hover:bg-red-50 rounded-lg transition" title="Hapus Permanen"><Trash size={16} /></button>
                          </div>
                        </td>
                      </tr>
                    ))
                    ...
                    pivotedDataInfo.data.map((row, idx) => (
                      <tr key={idx} className="hover:bg-blue-50/30 transition-colors group">
                        <td className="px-8 py-6 border-r border-gray-50 last:border-0">
                          <div className="text-sm font-black text-gray-900 group-hover:text-blue-600 transition-colors">{row.nama_penggunaan}</div>
                          {row.link_bukti && (
                            <a href={row.link_bukti} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 mt-1 text-blue-600 hover:text-blue-800 font-black text-[9px] uppercase tracking-widest">
                              ...
                              if (currentYearItem) {
                                return (
                                  <div className="inline-flex items-center bg-white border border-gray-200 p-1.5 rounded-xl shadow-sm transition-all group-hover:border-blue-200 group-hover:shadow-md">
                                    ...
                                    <div className="w-px h-4 bg-gray-200 mx-2"></div>
                                    <button onClick={() => handleSoftDelete(currentYearItem.id_penggunaan)} className="p-1.5 text-red-600 hover:bg-red-50 rounded-lg transition" title="Hapus TS"><Trash2 size={16} /></button>
                                  </div>
                                );
                              } else {
                                return (
