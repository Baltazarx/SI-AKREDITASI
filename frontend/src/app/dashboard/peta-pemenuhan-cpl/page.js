      ...
      <div className="max-w-7xl mx-auto p-6">
        ...
        <div className="mb-8">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              <h1 className="text-3xl font-black text-white tracking-tight">Peta Pemenuhan CPL (2.B.3)</h1>
              <p className="text-gray-400 mt-1 font-medium">Pengelolaan hubungan CPL, CPMK, dan Mata Kuliah</p>
            </div>
            <div className="flex flex-wrap gap-3">
              {showForm && (
                <button onClick={() => setShowForm(false)} className="flex items-center gap-2 bg-gray-800 text-gray-400 px-5 py-2.5 rounded-xl hover:bg-gray-700 transition font-bold text-sm">
                  <span>Tutup Form</span>
                </button>
              )}

              <button onClick={() => window.open(`http://localhost:5000/api/prodi/2b3-peta-pemenuhan/export?id_prodi=${filterProdi}&token=${localStorage.getItem('token')}`, '_blank')} className="flex items-center gap-2 bg-emerald-600 text-white px-5 py-2.5 rounded-xl hover:bg-emerald-700 transition shadow-lg shadow-emerald-900/20 font-bold text-sm">
                <Download size={18} />
        ...
        {showForm && (
          <div className="bg-gray-900 rounded-3xl shadow-xl shadow-gray-950/50 border border-gray-700 p-8 mb-8 animate-in slide-in-from-top-4 duration-500">
            <h2 className="text-xl font-black text-white mb-6">{isEditMode ? 'Atur Pemetaan Mata Kuliah' : 'Input Peta Pemenuhan CPL Baru'}</h2>
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className={`grid grid-cols-1 ${isEditMode ? 'md:grid-cols-3' : 'md:grid-cols-2'} gap-6`}>
                <div>
                  ...
                  <select disabled={isEditMode} value={formData.id_cpl} onChange={(e) => setFormData({...formData, id_cpl: e.target.value})} className="w-full px-4 py-3 bg-gray-950/50 border-transparent border-2 focus:border-blue-500 focus:bg-gray-900 rounded-2xl outline-none transition font-medium disabled:opacity-60" required>
                ...
                {isEditMode && (
                  <div>
                    <label className="block text-sm font-bold text-gray-300 mb-2">CPMK</label>
                    <select disabled={true} value={formData.id_cpmk} className="w-full px-4 py-3 bg-gray-950/50 border-transparent border-2 focus:border-blue-500 focus:bg-gray-900 rounded-2xl outline-none transition font-medium disabled:opacity-60 cursor-not-allowed text-white" required>
                ...
                <div>
                  ...
                  <select disabled={true} value={formData.id_tahun} onChange={(e) => setFormData({...formData, id_tahun: e.target.value})} className="w-full px-4 py-3 bg-gray-950/50 border-transparent border-2 focus:border-blue-500 focus:bg-gray-900 rounded-2xl outline-none transition font-medium disabled:opacity-60 cursor-not-allowed" required>
                    <option value="">Pilih Tahun</option>
                    {tahunList.map(t => <option key={t.id_tahun} value={t.id_tahun}>{t.tahun}</option>)}
                  </select>
                </div>
              ...
              {formData.id_cpl && (
                <div className="bg-gradient-to-br from-blue-950/50 to-indigo-900/20 border border-blue-900/60 rounded-2xl p-5 flex gap-4 items-start shadow-sm animate-in fade-in slide-in-from-top-2 duration-300">
                  <div className="bg-gray-900 p-2.5 rounded-xl shadow-sm border border-blue-900/50 text-blue-600 shrink-0">
                    <BookOpen size={20} />
                  </div>
                  <div>
                ...
                <div className="mt-6">
                  ...
                  {isEditMode ? (
                    ...
                    cpmkList.filter(c => c.id_cpmk.toString() === formData.id_cpmk.toString()).map(cpmk => (
                      <div key={cpmk.id_cpmk} className="bg-gray-950/50 p-5 rounded-2xl border border-gray-700 mb-4">
                        <div className="text-sm font-black text-gray-200 mb-3 border-b border-gray-700 pb-2">
                          {cpmk.kode_cpmk} <span className="text-gray-400 font-medium ml-2">{cpmk.deskripsi_cpmk}</span>
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 max-h-72 overflow-y-auto pr-2">
                          {mataKuliahList.map(mk => {
                            ...
                            return (
                              <label key={mk.id_mk} className={`flex items-start gap-3 p-3 rounded-xl border cursor-pointer transition-all ${isSelected ? 'bg-blue-900/80 border-blue-700 shadow-sm' : 'bg-gray-900 border-gray-700 hover:border-blue-800'}`}>
                                <div className="mt-0.5">
                                  <input type="checkbox" checked={isSelected} onChange={() => handleMkToggle(cpmk.id_cpmk, mk.id_mk)} className="w-4 h-4 text-blue-600 bg-gray-800 border-gray-600 rounded focus:ring-blue-500 focus:ring-2 cursor-pointer" />
                                </div>
                                <div>
                                  <div className={`text-xs font-black tracking-tight ${isSelected ? 'text-blue-200' : 'text-gray-200'}`}>{mk.kode_mk}</div>
                                  <div className="text-[10px] text-gray-400 mt-1 font-medium leading-tight">{mk.nama_mk}</div>
                                  <div className="inline-block mt-2 px-2 py-0.5 bg-gray-800 text-gray-400 rounded text-[9px] font-black uppercase">Semester {mk.semester}</div>
                                </div>
                              </label>
                            );
                          })}
                    ...
                    <div className="space-y-4 max-h-[600px] overflow-y-auto pr-2">
                      {cpmkList.map(cpmk => (
                        <div key={cpmk.id_cpmk} className="bg-gray-950/50 p-5 rounded-2xl border border-gray-700">
                          <div className="text-sm font-black text-gray-200 mb-3 border-b border-gray-700 pb-2">
                            {cpmk.kode_cpmk} <span className="text-gray-400 font-medium ml-2">{cpmk.deskripsi_cpmk}</span>
                          </div>
              ...
              <div className="flex justify-end gap-3 pt-4 border-t border-gray-700">
                ...
                <button type="submit" className="px-10 py-3 bg-blue-600 text-white rounded-2xl hover:bg-blue-700 transition font-black shadow-lg shadow-blue-900/20">{isEditMode ? 'Update Pemetaan' : 'Simpan Pemetaan'}</button>
              </div>
            </form>
          </div>
        )}
                ...
                return (
                  <table className="w-full text-left border-collapse">
                    <thead className="bg-gray-800 border-b-2 border-gray-700">
                      <tr>
                        <th className="px-4 py-3 text-[11px] font-black text-gray-300 uppercase tracking-widest border-r border-gray-700 bg-gray-800 align-middle sticky left-0 z-20 w-[260px] min-w-[260px] max-w-[260px] shadow-[inset_-1px_0_0_0_#1f2937]">CPL</th>
                        <th className="px-4 py-3 text-[11px] font-black text-gray-300 uppercase tracking-widest border-r border-gray-700 bg-gray-800 align-middle sticky left-[260px] z-20 w-[260px] min-w-[260px] max-w-[260px] shadow-[inset_-1px_0_0_0_#1f2937]">CPMK</th>
                        {displaySemesters.map(sem => (
                          <th key={sem} className="px-4 py-3 text-[11px] font-black text-gray-300 uppercase tracking-widest border-r border-gray-700 text-center bg-gray-950/50 align-middle w-[160px] min-w-[160px] max-w-[160px]">
                            Semester {sem}
                          </th>
                        ))}
                      </tr>
                ...
                <tbody className="divide-y divide-gray-800">
                  {cplList.map(cpl => {
                    ...
                    if (cpmkArray.length === 0) {
                      return (
                        <tr key={cpl.id_cpl} className="hover:bg-blue-900/30 transition-colors group">
                          <td className="px-4 py-3 border-r border-gray-700 bg-gray-900 group-hover:bg-gray-800 align-top sticky left-0 z-10 w-[260px] min-w-[260px] max-w-[260px] shadow-[inset_-1px_0_0_0_#1f2937] transition-colors">
                          ...
                          <td className="px-4 py-3 border-r border-gray-700 align-middle text-center bg-gray-900 group-hover:bg-gray-800 sticky left-[260px] z-10 w-[260px] min-w-[260px] max-w-[260px] shadow-[inset_-1px_0_0_0_#1f2937] transition-colors">
                            <div className="opacity-50 group-hover:opacity-100 transition-opacity">
                              <button onClick={() => handleAddMapping(cpl)} className="inline-flex items-center gap-1.5 px-4 py-2 bg-blue-900/20 text-blue-600 text-[10px] font-black uppercase tracking-wider rounded-lg hover:bg-blue-600 hover:text-white transition-colors border border-blue-900/50 shadow-sm">
                                <Plus size={14} />
                                Tambah Pemetaan
                              </button>
                            </div>
                          ...
                          {displaySemesters.map(sem => (
                            <td key={sem} className="px-4 py-3 border-r border-gray-700 text-center w-[160px] min-w-[160px] max-w-[160px]"></td>
                          ))}
                        </tr>
                      );
                    }
                    ...
                    return cpmkArray.map((cpmkGroup, index) => (
                      <tr key={`${cpl.id_cpl}-${cpmkGroup.id_cpmk}`} className="hover:bg-blue-900/30 transition-colors group">
                        {index === 0 && (
                          <td rowSpan={cpmkArray.length} className="px-4 py-3 border-r border-gray-700 bg-gray-900 group-hover:bg-gray-800 align-top sticky left-0 z-10 w-[260px] min-w-[260px] max-w-[260px] shadow-[inset_-1px_0_0_0_#1f2937] transition-colors">
                            <div className="font-black text-white text-sm">{cpl.kode_cpl || '-'}</div>
                            <div className="text-[10px] text-gray-400 mt-1 font-medium">{cpl.deskripsi_cpl || '-'}</div>
                          </td>
                        )}
                        <td className="px-4 py-3 border-r border-gray-700 align-top bg-gray-900 group-hover:bg-gray-800 sticky left-[260px] z-10 w-[260px] min-w-[260px] max-w-[260px] shadow-[inset_-1px_0_0_0_#1f2937] transition-colors group">
                          <div className="font-bold text-gray-200 text-sm">{cpmkGroup.cpmkKode}</div>
                          <div className="text-[10px] text-gray-400 mt-1 mb-6">{cpmkGroup.cpmkDesc}</div>
                          
                          <div className="mt-3">
                            <button onClick={() => handleEditGroup(cpl, cpmkGroup)} className="inline-flex items-center gap-1.5 px-3 py-1.5 w-full justify-center bg-blue-900/20 text-blue-600 text-[10px] font-black uppercase tracking-wider rounded-lg hover:bg-blue-600 hover:text-white transition-colors border border-blue-900/50 shadow-sm opacity-0 group-hover:opacity-100 focus:opacity-100">
                              <Edit size={12} />
                              Atur MK
                            </button>
                          </div>
                        ...
                        {displaySemesters.map(sem => {
                          ...
                          return (
                            <td key={sem} className="px-4 py-3 border-r border-gray-700 align-top w-[160px] min-w-[160px] max-w-[160px]">
                              {mksInSem.length > 0 ? (
                                <div className="flex flex-col gap-2">
                                  {mksInSem.map(mk => (
                                    <div key={mk.id_2b3} className="bg-blue-900/80 border border-blue-800 rounded-lg p-2.5 transition-colors hover:bg-blue-900/80">
                                      ...
                                      <div className="text-[10px] text-blue-400 leading-tight mt-0.5">{mk.nama_mk || '-'}</div>
                                    </div>
                                  ))}
                                </div>
                              ) : null}
