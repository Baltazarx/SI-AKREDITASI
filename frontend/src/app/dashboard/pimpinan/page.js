      ...
      <div className="max-w-7xl mx-auto p-6">
        ...
        <div className="mb-8">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              <h1 className="text-3xl font-black text-white tracking-tight">Pimpinan (1.A.1)</h1>
              <p className="text-gray-400 mt-1 font-medium">Kelola data pimpinan UPPS & Tupoksi</p>
            </div>
        ...
        <div className="flex flex-col lg:flex-row gap-6 mb-8">
          <div className="flex-1 grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-gray-900 p-5 rounded-2xl shadow-sm border border-gray-700 flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-2xl font-black text-white">{isTrashMode ? '-' : data.length}</p>
              </div>
            </div>
            <div className="bg-gray-900 p-5 rounded-2xl shadow-sm border border-gray-700 flex items-center gap-4">
              <div className="p-3 bg-purple-900/20 text-purple-600 rounded-xl"><Briefcase size={24} /></div>
              <div>
                <p className="text-xs font-bold text-gray-400 uppercase tracking-wider">Unit UPPS</p>
                <p className="text-2xl font-black text-white">{isTrashMode ? '-' : [...new Set(data.map(d => d.nama_unit_display))].length}</p>
              </div>
            </div>
            <div className="bg-gray-900 p-5 rounded-2xl shadow-sm border border-gray-700 flex items-center gap-4">
          ...
          <div className="flex gap-3 items-end">
            <button onClick={() => fetchData(isTrashMode)} className="p-2.5 bg-gray-900 border border-gray-700 rounded-xl hover:bg-gray-950/50 transition text-gray-400 hover:text-blue-600 shadow-sm" title="Refresh Data">
            ...
            <button 
              ...
              className={`px-4 py-2.5 rounded-xl font-bold text-sm transition border shadow-sm ${isTrashMode ? 'bg-orange-900/20 border-orange-800 text-orange-600' : 'bg-gray-900 border-gray-700 text-gray-400 hover:bg-gray-950/50'}`}
            >
        ...
        {showForm && (
          <div className="bg-gray-900 rounded-3xl shadow-xl shadow-gray-950/50 border border-gray-700 p-8 mb-8 animate-in slide-in-from-top-4 duration-500">
            <h2 className="text-xl font-black text-white mb-6">{editingId ? 'Edit Data Pimpinan' : 'Input Pimpinan Baru'}</h2>
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <input type="text" value={formData.nama_jafung_display} readOnly className="w-full px-4 py-3 bg-gray-800 border-2 border-transparent rounded-2xl outline-none font-medium text-gray-400 cursor-not-allowed" />
                </div>
                <div>
                  <label className="block text-sm font-bold text-gray-300 mb-2">Periode Mulai</label>
                  <div className="relative">
                    <Calendar className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400" size={18} />
                    <input type="number" value={formData.periode_mulai} onChange={(e) => setFormData({ ...formData, periode_mulai: e.target.value })} className="w-full pl-12 pr-4 py-3 bg-gray-950/50 border-transparent border-2 focus:border-blue-500 focus:bg-gray-900 rounded-2xl outline-none transition font-medium" placeholder="Tahun Mulai (e.g. 2023)" />
                  </div>
                </div>
                <div>
                  <label className="block text-sm font-bold text-gray-300 mb-2">Periode Selesai</label>
                  <div className="relative">
                    <Calendar className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400" size={18} />
                    <input type="number" value={formData.periode_selesai} onChange={(e) => setFormData({ ...formData, periode_selesai: e.target.value })} className="w-full pl-12 pr-4 py-3 bg-gray-950/50 border-transparent border-2 focus:border-blue-500 focus:bg-gray-900 rounded-2xl outline-none transition font-medium" placeholder="Tahun Selesai (e.g. 2026)" />
                  </div>
                </div>
                <div className="md:col-span-2">
                  <label className="block text-sm font-bold text-gray-300 mb-2">Tupoksi (Tugas Pokok & Fungsi)</label>
                  <textarea value={formData.tupoksi} onChange={(e) => setFormData({ ...formData, tupoksi: e.target.value })} className="w-full px-4 py-3 bg-gray-950/50 border-transparent border-2 focus:border-blue-500 focus:bg-gray-900 rounded-2xl outline-none transition font-medium" rows="3" placeholder="Masukkan rincian tupoksi..." />
                </div>
              </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left">
                <thead className="bg-gray-800 border-b border-gray-700">
                  <tr>
                    <th className="px-8 py-5 text-[11px] font-black text-gray-300 uppercase tracking-[0.2em] border-r border-gray-700 last:border-0">Unit / Jabatan</th>
                    <th className="px-8 py-5 text-[11px] font-black text-gray-300 uppercase tracking-[0.2em] border-r border-gray-700 last:border-0">Nama Pimpinan</th>
                    <th className="px-8 py-5 text-[11px] font-black text-gray-300 uppercase tracking-[0.2em] border-r border-gray-700 last:border-0 text-center">Periode</th>
                    <th className="px-8 py-5 text-[11px] font-black text-gray-300 uppercase tracking-[0.2em] border-r border-gray-700 last:border-0">Pendidikan Terakhir</th>
                    <th className="px-8 py-5 text-[11px] font-black text-gray-300 uppercase tracking-[0.2em] border-r border-gray-700 last:border-0">Tupoksi</th>
                    <th className="px-8 py-5 text-[11px] font-black text-gray-300 uppercase tracking-[0.2em]">Aksi</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-800">
                  {data.map((item) => (
                    <tr key={item.id_pimpinan} className="hover:bg-blue-900/30 transition-colors group">
                      <td className="px-8 py-6 border-r border-gray-700 last:border-0">
                        <div className="text-sm font-black text-white group-hover:text-blue-600 transition-colors">{item.nama_unit_display || '-'}</div>
                      </td>
                      <td className="px-8 py-6 border-r border-gray-700 last:border-0">
                        <div className="text-sm font-bold text-gray-200">{item.nama_lengkap || '-'}</div>
                        <div className="text-[10px] text-gray-400 font-bold mt-0.5 uppercase">{item.nama_jafung || 'Non-Jafung'}</div>
                      </td>
                      <td className="px-8 py-6 border-r border-gray-700 last:border-0">
                        <span className="px-3 py-1 bg-blue-900/20 text-blue-600 inline-block text-[10px] font-black tracking-wider whitespace-nowrap">{item.periode_mulai} - {item.periode_selesai}</span>
                      </td>
                      <td className="px-8 py-6 border-r border-gray-700 last:border-0">
                        <div className="text-xs font-medium text-gray-400 uppercase tracking-wider">{item.pendidikan_terakhir || '-'}</div>
                      </td>
                      <td className="px-8 py-6 border-r border-gray-700 last:border-0">
                        <div className="text-xs font-medium text-gray-400 line-clamp-2 max-w-xs">{item.tupoksi || '-'}</div>
                      </td>
                      <td className="px-8 py-6">
                        <div className="flex flex-wrap gap-2">
                          {isTrashMode ? (
                            <div className="inline-flex items-center bg-gray-900 border border-gray-700 p-1.5 rounded-xl shadow-sm transition-all group-hover:border-orange-800">
                              <button onClick={() => handleRestore(item.id_pimpinan)} className="p-1.5 text-emerald-600 hover:bg-emerald-900/20 rounded-lg transition" title="Restore"><History size={16} /></button>
                              <div className="w-px h-4 bg-gray-700 mx-2"></div>
                              <button onClick={() => handleHardDelete(item.id_pimpinan)} className="p-1.5 text-red-600 hover:bg-red-950/40 rounded-lg transition" title="Hapus Permanen"><Trash2 size={16} /></button>
                            </div>
                          ) : (
                            <div className="inline-flex items-center bg-gray-900 border border-gray-700 p-1.5 rounded-xl shadow-sm transition-all group-hover:border-blue-800 group-hover:shadow-md">
                              ...
                              <div className="w-px h-4 bg-gray-700 mx-2"></div>
                              <button onClick={() => handleDelete(item.id_pimpinan)} className="p-1.5 text-red-600 hover:bg-red-950/40 rounded-lg transition" title="Hapus"><Trash2 size={16} /></button>
                            </div>
                          )}
                        </div>
                      </td>
