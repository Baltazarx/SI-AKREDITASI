...
export default function PkmPage() {
  ...
  const fetchRoadmap = async () => {
    ...
    const token = localStorage.getItem('token');
    try {
      const res = await fetch(`http://localhost:5000/api/lppm/roadmap-lppm?id_prodi=${filterIdProdi}&id_tahun=${filterIdTahun}&jenis=PkM`, { headers: { 'Authorization': `Bearer ${token}` } });
  ...
  return (
    <div className="p-6">
      <div className="max-w-7xl mx-auto">
        <div className="mb-8">
          <div>
            <h1 className="text-3xl font-black text-white dark:text-white tracking-tight">(4.A.2) PkM DTPR, Hibah dan Pembiayaan PkM</h1>
            <p className="text-gray-400 dark:text-gray-400 mt-1 font-medium">Monitoring Hibah dan Pembiayaan PkM beserta Tabel Relasi 4.C</p>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="space-y-6">
            ...
            <div className="bg-gray-900 dark:bg-gray-900 border-2 border-orange-900/50 dark:border-orange-900/30 p-6 rounded-3xl relative overflow-hidden shadow-sm">
              <div className="absolute top-0 right-0 w-64 h-64 bg-orange-900/20 dark:bg-orange-900/10 rounded-full blur-3xl -mr-20 -mt-20"></div>
              <div className="relative z-10 flex flex-col md:flex-row items-center gap-6">
                ...
                <div className="hidden md:flex flex-col items-center justify-center relative w-16">
                  <div className="w-full h-1 bg-orange-200 dark:bg-orange-800 rounded-full relative">
                    <div className="absolute right-0 top-1/2 -translate-y-1/2 w-3 h-3 bg-orange-900/200 rounded-full animate-ping"></div>
                    <div className="absolute right-0 top-1/2 -translate-y-1/2 w-3 h-3 bg-orange-600 rounded-full"></div>
                  </div>
                  <span className="text-[10px] font-black text-orange-500 uppercase tracking-widest absolute -top-5 bg-gray-900 dark:bg-gray-900 px-2">Flow</span>
                </div>

                {/* PkM Node (Modified to Mini-Table) */}
                <div className="flex-[1.5] bg-gray-900 dark:bg-gray-900 border-2 border-dashed border-orange-800 dark:border-orange-800 p-5 rounded-3xl relative overflow-hidden group">
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-3">
                      <div className="p-2 bg-orange-900/30 dark:bg-orange-900/40 rounded-xl"><BookOpen size={20} className="text-orange-600 dark:text-orange-400" /></div>
                      <span className="text-[11px] font-black uppercase tracking-widest text-orange-500">Kaitan Data PkM</span>
                    </div>
                    <div className="px-3 py-1 bg-orange-900/20 dark:bg-orange-900/30 rounded-full text-[10px] font-bold text-orange-600">{pkmData.length} Judul</div>
                  </div>
                  
                  <div className="grid grid-cols-4 gap-2">
                    <button 
                      onClick={() => setActiveSubTab('4a2')}
                      className={`flex flex-col items-center p-3 rounded-2xl transition-all border ${activeSubTab === '4a2' ? 'bg-orange-600 border-orange-600 text-white shadow-lg' : 'bg-orange-900/20 dark:bg-orange-900/10 border-orange-900/50 dark:border-orange-900/30 text-orange-400 dark:text-orange-300 hover:scale-105'}`}
                    >
                    ...
                    <button 
                      ...
                      className={`flex flex-col items-center p-3 rounded-2xl transition-all border ${activeSubTab === '4c1' ? 'bg-purple-600 border-purple-600 text-white shadow-lg' : 'bg-purple-900/20 dark:bg-purple-900/10 border-purple-900/50 dark:border-purple-900/30 text-purple-400 dark:text-purple-300 hover:scale-105'}`}
                    >
                    ...
                    <button 
                      ...
                      className={`flex flex-col items-center p-3 rounded-2xl transition-all border ${activeSubTab === '4c2' ? 'bg-orange-600 border-orange-600 text-white shadow-lg' : 'bg-orange-900/20 dark:bg-orange-900/10 border-orange-900/50 dark:border-orange-900/30 text-orange-400 dark:text-orange-300 hover:scale-105'}`}
                    >
                    ...
                    <button 
                      ...
                      className={`flex flex-col items-center p-3 rounded-2xl transition-all border ${activeSubTab === '4c3' ? 'bg-emerald-600 border-emerald-600 text-white shadow-lg' : 'bg-emerald-900/20 dark:bg-emerald-900/10 border-emerald-900/50 dark:border-emerald-900/30 text-emerald-400 dark:text-emerald-300 hover:scale-105'}`}
                    >
                      ...
                      <span className="text-[9px] font-bold uppercase mt-1">HKI PkM</span>
                    </button>
                  </div>
                </div>
              </div>
            ...
            {!showForm && (
              <div className="flex flex-col md:flex-row justify-between items-center gap-4">
                ...
                <div className="flex bg-gray-800 dark:bg-gray-800 p-1 rounded-2xl w-full md:w-auto">
                  <button 
                    onClick={() => setActiveSubTab('4a2')}
                    className={`px-4 py-2 rounded-xl text-xs font-black transition-all ${activeSubTab === '4a2' ? 'bg-gray-900 dark:bg-gray-700 shadow-sm text-orange-600' : 'text-gray-400 hover:text-gray-300 dark:hover:text-gray-300'}`}
                  >
                    Tabel 4.A.2
                  </button>
                  <button 
                    onClick={() => setActiveSubTab('4c1')}
                    className={`px-4 py-2 rounded-xl text-xs font-black transition-all ${activeSubTab === '4c1' ? 'bg-gray-900 dark:bg-gray-700 shadow-sm text-purple-600' : 'text-gray-400 hover:text-gray-300 dark:hover:text-gray-300'}`}
                  >
                    Tabel 4.C.1 Kerjasama
                  </button>
                  <button 
                    onClick={() => setActiveSubTab('4c2')}
                    className={`px-4 py-2 rounded-xl text-xs font-black transition-all ${activeSubTab === '4c2' ? 'bg-gray-900 dark:bg-gray-700 shadow-sm text-orange-600' : 'text-gray-400 hover:text-gray-300 dark:hover:text-gray-300'}`}
                  >
                    Tabel 4.C.2 Diseminasi
                  </button>
                  <button 
                    onClick={() => setActiveSubTab('4c3')}
                    className={`px-4 py-2 rounded-xl text-xs font-black transition-all ${activeSubTab === '4c3' ? 'bg-gray-900 dark:bg-gray-700 shadow-sm text-emerald-600' : 'text-gray-400 hover:text-gray-300 dark:hover:text-gray-300'}`}
                  >
                    Tabel 4.C.3 HKI
                  </button>
                </div>
            ...
            {showForm && (
              <div className="bg-gray-900 dark:bg-gray-900 p-8 rounded-3xl shadow-xl border border-gray-700 dark:border-gray-700 animate-in slide-in-from-bottom-4 duration-500">
                <h2 className="text-2xl font-black text-orange-600 dark:text-orange-400 mb-6 border-b pb-4 border-gray-700 dark:border-gray-700 flex items-center gap-2">
                ...
                <form onSubmit={handleSubmit} className="space-y-8">
                  ...
                  <div className="space-y-4">
                    <h3 className="font-bold text-orange-600 dark:text-orange-400 text-lg flex items-center gap-2"><BookOpen size={20} /> 1. Data Induk (4.A.2 PkM DTPR)</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-gray-950/50 dark:bg-gray-800/50 p-6 rounded-2xl border border-gray-700 dark:border-gray-700">
                      ...
                      <div>
                        <label className="block text-xs font-bold text-gray-300 dark:text-gray-300 mb-1">Judul PkM</label>
                      ...
                      <div>
                        <label className="block text-xs font-bold text-gray-300 dark:text-gray-300 mb-1">Sumber Pembiayaan</label>
                  ...
                  <div className="space-y-4">
                    <div className="flex justify-between items-center">
                      <h3 className="font-bold text-purple-600 dark:text-purple-400 text-lg flex items-center gap-2"><Users size={20} /> 2. Kerjasama PkM (Tabel 4.C.1)</h3>
                  ...
                  <div className="space-y-4">
                    <div className="flex justify-between items-center">
                      <h3 className="font-bold text-orange-600 dark:text-orange-400 text-lg flex items-center gap-2"><BookOpen size={20} /> 3. Diseminasi Hasil PkM (Tabel 4.C.2)</h3>
                  ...
                  <div className="space-y-4">
                    <div className="flex justify-between items-center">
                      <h3 className="font-bold text-emerald-600 dark:text-emerald-400 text-lg flex items-center gap-2"><Activity size={20} /> 4. Perolehan HKI PkM (Tabel 4.C.3)</h3>
                  ...
                  <div className="flex justify-end gap-3 pt-6 border-t dark:border-gray-700">
                    ...
                    <button type="submit" className="px-8 py-2.5 bg-orange-600 text-white rounded-xl font-black shadow-lg shadow-orange-950/50 dark:shadow-orange-900/20 hover:bg-orange-700 transition">{editingId ? 'Simpan Perubahan' : 'Simpan Data'}</button>
                  </div>
                </form>
              </div>
            )}
                      ...
                      <table className="w-full border-collapse text-sm">
                        <thead>
                          <tr className="bg-gray-800 dark:bg-gray-800 text-[11px] font-black text-gray-300 dark:text-gray-300 uppercase text-center">
                            ...
                            <th rowSpan="2" className="px-4 py-3 border border-gray-600 dark:border-gray-700 text-left min-w-[200px]">Judul PkM</th>
                        ...
                        <tbody>
                          {pkmData.length === 0 ? (
                            ...
                            pkmData.map((item, idx) => {
                              ...
                              return (
                                <tr key={item.id_4a2} className="hover:bg-orange-900/50 dark:hover:bg-orange-900/10 transition-colors text-center text-gray-300 dark:text-gray-300">
                                  ...
                                  <td className="px-4 py-3 border border-gray-600 dark:border-gray-700 text-left">
                                    <div className="font-semibold text-white dark:text-white leading-snug">{item.judul_pkm}</div>
                                  ...
                                  <td className="px-4 py-3 border border-gray-600 dark:border-gray-700 font-black text-orange-600">{formatSumberShort(item.sumber)}</td>
                                  ...
                                  <td className="px-4 py-3 border border-gray-600 dark:border-gray-700">
                                    <div className="flex items-center justify-center gap-1">
                                      ...
                                      <button onClick={() => handleDelete(item.id_4a2)} className="p-1.5 bg-red-950/40 dark:bg-red-900/30 text-red-600 dark:text-red-400 rounded-lg hover:bg-red-600 hover:text-white transition" title="Hapus"><Trash2 size={14} /></button>
                                    </div>
                                  </td>
                                </tr>
                              )
                            ...
                            pkmData.flatMap(p => (p.kerjasama || []).map(k => ({ ...k, nama_dosen: p.nama_dosen, judul_pkm: p.judul_pkm }))).map((item, idx) => (
                              <tr key={idx} className="hover:bg-purple-900/30 dark:hover:bg-purple-900/5 transition-colors text-center text-gray-300 dark:text-gray-300">
                                ...
                                <td className="px-4 py-3 border border-purple-900/50 dark:border-purple-900/30 text-left">
                                  ...
                                  <div className="text-[10px] text-gray-400 dark:text-gray-400 flex items-center gap-1">Ref: {item.judul_pkm}</div>
                                </td>
                        ...
                        <tbody>
                          {pkmData.every(p => !p.hki?.length) ? (
                            ...
                            pkmData.flatMap(p => (p.hki || []).map(h => ({ ...h, nama_dosen: p.nama_dosen, judul_pkm: p.judul_pkm }))).map((item, idx) => (
                              <tr key={idx} className="hover:bg-emerald-900/30 dark:hover:bg-orange-900/5 transition-colors text-center text-gray-300 dark:text-gray-300">
