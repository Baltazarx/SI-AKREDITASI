      ...
      <div className="max-w-7xl mx-auto p-6">
        ...
        <div className="mb-8">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              <h1 className="text-3xl font-black text-white tracking-tight">Sarpras PkM (4.A.1)</h1>
              <p className="text-gray-400 mt-1 font-medium">Tabel 4.A.1 Sarana dan Prasarana PkM</p>
            </div>
        ...
        {error && (
          <div className="bg-red-950/40 border border-red-800 text-red-600 px-4 py-3 rounded-xl mb-6 animate-in fade-in duration-300 font-medium">
            {error}
          </div>
        )}
        ...
        <div className="flex flex-col lg:flex-row gap-6 mb-8">
          <div className="flex-1 grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-gray-900 p-5 rounded-2xl shadow-sm border border-gray-700 flex items-center gap-4">
              <div className="p-3 bg-blue-900/20 text-blue-600 rounded-xl"><Building2 size={24} /></div>
              <div>
            ...
            <div className="bg-gray-900 p-5 rounded-2xl shadow-sm border border-gray-700 flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-2xl font-black text-white">{activeData.reduce((acc, curr) => acc + (parseFloat(curr.luas_ruang) || 0), 0).toFixed(1)} <span className="text-sm font-bold text-gray-400">m²</span></p>
              </div>
            </div>
            <div className="bg-gray-900 p-5 rounded-2xl shadow-sm border border-gray-700 flex items-center gap-4">
        ...
        {showForm && (
          <div className="bg-gray-900 rounded-3xl shadow-xl shadow-gray-950/50 border border-gray-700 p-8 mb-8 animate-in slide-in-from-top-4 duration-500">
            <div className="flex items-center gap-3 mb-6">
              ...
              <h2 className="text-xl font-black text-white">{editingId ? 'Edit Data Sarpras' : 'Input Sarpras Baru'}</h2>
            </div>
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <select 
                      ...
                      className="w-full px-4 py-3 bg-gray-950/50 border-transparent border-2 focus:border-blue-500 focus:bg-gray-900 rounded-2xl outline-none transition font-medium text-sm"
                    >
                ...
                <div className="md:col-span-3">
                  <label className="block text-sm font-bold text-gray-300 mb-2">Detail Perangkat</label>
                  <textarea 
              ...
              <div className="flex justify-end gap-3 pt-4">
                ...
                <button type="submit" className="px-10 py-3 bg-blue-600 text-white rounded-2xl hover:bg-blue-700 transition font-black shadow-lg shadow-blue-900/20">{editingId ? 'Update Data' : 'Simpan Sarpras'}</button>
              </div>
            </form>
          </div>
        )}
        ...
        <div className="bg-gray-900 rounded-[2.5rem] shadow-xl shadow-gray-950/30 border border-gray-700 overflow-hidden transition-all duration-500">
          {loading ? (
            ...
            <div className="p-20 text-center">
              <div className="w-20 h-20 bg-gray-950/50 rounded-full flex items-center justify-center mx-auto mb-4 text-gray-300">
                <Layout size={40} />
              </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left">
                <thead className="bg-gray-800 border-b border-gray-700">
                  <tr>
                    <th className="px-6 py-5 text-[10px] font-black text-gray-300 uppercase tracking-wider border">Nama Prasarana</th>
                    <th className="px-4 py-5 text-[10px] font-black text-gray-300 uppercase tracking-wider border text-center">Daya Tampung</th>
                    <th className="px-4 py-5 text-[10px] font-black text-gray-300 uppercase tracking-wider border text-center">Luas Ruang (m²)</th>
                    <th className="px-6 py-5 text-[10px] font-black text-gray-300 uppercase tracking-wider border text-center">Milik Sendiri (M)/ Sewa (W)</th>
                    <th className="px-6 py-5 text-[10px] font-black text-gray-300 uppercase tracking-wider border text-center">Berlisensi (L)/ Public Domain (P)/ Tidak Berlisensi (T)</th>
                    <th className="px-6 py-5 text-[10px] font-black text-gray-300 uppercase tracking-wider border">Perangkat</th>
                    <th className="px-6 py-5 text-[10px] font-black text-gray-300 uppercase tracking-wider border text-center">Info Tambahan</th>
                    <th className="px-6 py-5 text-[10px] font-black text-gray-300 uppercase tracking-wider border">Link Bukti</th>
                    {!showTrash && <th className="px-6 py-5 text-[10px] font-black text-gray-300 uppercase tracking-wider border">Aksi</th>}
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-800">
                  {(showTrash ? trashData : activeData).map((item) => (
                    <tr key={item.id_4a1} className="hover:bg-blue-900/30 transition-colors group">
                      <td className="px-6 py-6 border text-sm font-black text-white group-hover:text-blue-600 transition-colors">{item.nama_prasarana}</td>
                      <td className="px-4 py-6 border text-center">
                        <span className="inline-block px-3 py-1 bg-gray-800 text-gray-400 rounded-full text-xs font-black">{item.daya_tampung}</span>
                      </td>
                      <td className="px-4 py-6 border text-center">
                        <div className="text-sm font-bold text-white">{item.luas_ruang}</div>
                      </td>
                      <td className="px-6 py-6 border text-center">
                        <span className={`px-3 py-1 rounded-lg text-xs font-black uppercase ${item.status_milik === 'M' ? 'bg-emerald-900/30 text-emerald-600' : 'bg-orange-900/30 text-orange-600'}`}>
                          {item.status_milik}
                        </span>
                      </td>
                      <td className="px-6 py-6 border text-center">
                        <span className={`px-3 py-1 rounded-lg text-xs font-black uppercase ${item.status_lisensi === 'L' ? 'bg-blue-900/30 text-blue-600' : item.status_lisensi === 'P' ? 'bg-purple-900/30 text-purple-600' : 'bg-gray-800 text-gray-400'}`}>
                          {item.status_lisensi}
                        </span>
                      </td>
                      <td className="px-6 py-6 border text-xs text-gray-400 font-medium max-w-xs truncate">{item.perangkat || '-'}</td>
                      <td className="px-6 py-6 border text-center text-xs text-gray-400 font-medium">{item.info_tambahan || '-'}</td>
                      <td className="px-6 py-6 border">
                        {item.link_bukti ? (
                          <a href={item.link_bukti} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 text-blue-600 hover:text-blue-300 font-black text-xs uppercase tracking-wider">
                      ...
                      {!showTrash && (
                        <td className="px-6 py-6 border">
                          <div className="inline-flex items-center bg-gray-900 border border-gray-700 p-1.5 rounded-xl shadow-sm">
                            ...
                            <button onClick={() => handleSoftDelete(item.id_4a1)} className="p-1.5 text-red-600 hover:bg-red-950/40 rounded-lg transition" title="Hapus"><Trash2 size={16} /></button>
                          </div>
                        </td>
                      )}
                      {showTrash && (
                         <td className="px-6 py-6 border">
                           <div className="inline-flex items-center bg-gray-900 border border-gray-700 p-1.5 rounded-xl shadow-sm">
                             <button onClick={() => handleRestore(item.id_4a1)} className="p-1.5 text-emerald-600 hover:bg-emerald-900/20 rounded-lg transition" title="Restore"><RotateCcw size={16} /></button>
                             <div className="w-px h-4 bg-gray-700 mx-2"></div>
                             <button onClick={() => handleHardDelete(item.id_4a1)} className="p-1.5 text-red-600 hover:bg-red-950/40 rounded-lg transition" title="Hapus Permanen"><Trash size={16} /></button>
                           </div>
                         </td>
                      )}
                    </tr>
